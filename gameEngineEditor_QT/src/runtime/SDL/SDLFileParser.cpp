...
uint32_t GameEngineEditor::SDLFileParser::parseFile(const std::filesystem::path& filePath, const GameEngine::MousePosition &mousePosition)
{
    std::string extension = filePath.extension().u8string();
    if (extension == ".asset")
    {
        return SDLFileParser::parseAssetFile(filePath, mousePosition);
    }
    else
    {
        // unknow file type
...
uint32_t GameEngineEditor::SDLFileParser::parseAssetFile(const std::filesystem::path& filePath, const GameEngine::MousePosition &mousePosition)
{
    Json json;
    std::ifstream input(filePath.u8string());
    input >> json;
    GameEngine::Actor* actor = GameEngine::globalScene->spawnActor<GameEngine::Actor>();
