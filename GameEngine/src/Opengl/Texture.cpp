...
void GameEngine::Texture::load(const char* const path, int filter)
{
    ...
    glTextureSubImage2D(this->id, 0, 0, 0, this->width, this->height, this->dataFormat, GL_UNSIGNED_BYTE, this->pixels);
...
{
    glTextureSubImage2D(this->id, 0, 0, 0, this->width, this->height, this->dataFormat, GL_UNSIGNED_BYTE, data);
